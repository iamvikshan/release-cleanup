...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --github-repo)
      if [[ $# -lt 2 || -z "${2-}" || "${2-}" == -* ]]; then
        echo "Error: --github-repo requires a repository URL argument."
        echo "Run '$SCRIPT_NAME --help' for usage information."
        exit 1
      fi
...
# Validate GITHUB_MIRROR_REPO format
if [[ ! "$GITHUB_MIRROR_REPO" =~ ^(https://|git@) ]]; then
  echo "Error: GITHUB_MIRROR_REPO must start with 'https://' or 'git@'"
  echo "   Provided: $GITHUB_MIRROR_REPO"
  exit 1
fi
...
# - 'origin' -> GitLab (primary, required for GitLab Duo Agent Platform in VS Code)
# - 'github' -> GitHub (secondary mirror for sync)
echo "Ensuring git remotes are configured..."
# Check if we are in a git repo
if git rev-parse --is-inside-work-tree > /dev/null 2>&1; then

  # --- Configure 'origin' remote (GitLab - primary) ---
  echo ""
  echo "Configuring 'origin' remote (GitLab - primary)..."
  if git remote | grep -q "^origin$"; then
    ...
    if [ "$NORMALIZED_CURRENT" != "$NORMALIZED_TARGET" ]; then
      ...
      if [[ "$PROCEED_WITH_UPDATE" = "true" ]]; then
        # If current origin points to GitHub, preserve it as 'github' remote before overwriting
        if [[ "$CURRENT_URL" == *"github.com"* ]]; then
          if ! git remote | grep -q "^github$"; then
            echo "Preserving old origin as 'github' remote..."
            git remote add github "$CURRENT_URL"
            echo "✓ GitHub remote preserved as 'github'"
          fi
        fi
        echo "Updating 'origin' remote to $TARGET_REPO..."
        git remote set-url origin "$TARGET_REPO"
        echo "✓ Remote 'origin' updated to GitLab"
      fi
    else
    ...
    echo "✓ Remote 'origin' added (GitLab)"
  fi

  # --- Configure 'github' remote (mirror) ---
  echo ""
  echo "Configuring 'github' remote (mirror)..."
  if git remote | grep -q "^github$"; then
    CURRENT_GH_URL=$(git remote get-url github)
    NORMALIZED_GH_CURRENT="${CURRENT_GH_URL%.git}"
    NORMALIZED_GH_CURRENT="${NORMALIZED_GH_CURRENT%/}"
    NORMALIZED_GH_TARGET="${GITHUB_MIRROR_REPO%.git}"
    NORMALIZED_GH_TARGET="${NORMALIZED_GH_TARGET%/}"
    if [ "$NORMALIZED_GH_CURRENT" != "$NORMALIZED_GH_TARGET" ]; then
      echo "Updating 'github' remote to $GITHUB_MIRROR_REPO..."
      git remote set-url github "$GITHUB_MIRROR_REPO"
      echo "✓ Remote 'github' updated"
    else
      echo "✓ Remote 'github' is already set to $CURRENT_GH_URL"
    fi
  else
    echo "Adding 'github' remote..."
    git remote add github "$GITHUB_MIRROR_REPO"
    echo "✓ Remote 'github' added (mirror)"
  fi

  echo ""
  echo "Remote configuration:"
  echo "   origin (primary): $(git remote get-url origin 2>/dev/null || echo 'not set')"
  echo "   github (mirror):  $(git remote get-url github 2>/dev/null || echo 'not set')"
  echo ""
